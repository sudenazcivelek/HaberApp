...
class _NewsPageState extends State<NewsPage> {
  List<Category> categories = [
    ...
    Category('entertainment', 'ENTERTAIMENT'),
    Category('general', 'GENERAL'),
    Category('health', 'HEALTH'),
    Category('science', 'SCİENCE'),
  ...
  List<GestureDetector> getCategoriesTab(ArticleListViewModel vm) {
    ...
    for (int i = 0; i < categories.length; i++) {
      ...
      )); //Yatay yönlü lisste oluşturur apbar için
    }
    return list;
  }
  ...
  Widget getWidgetByStatus(ArticleListViewModel vm) {
    switch (vm.status.index) {
      case 2:
        return Expanded(
            //tüm alanda kullanılması sağlanır
            child: ListView.builder(
                //kategoriye göre haber gösterir. kart yapısı
                itemCount: vm.viewModel.articles.length, //öğe sayısı
                itemBuilder: (context, index) {
                  //her öğe için widget oluşturur
                  return Card(
                    child: Column(
                      children: [
